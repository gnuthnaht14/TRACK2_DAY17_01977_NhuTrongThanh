...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    """Upsert nguyên tử một lô message theo event_id."""
    columns = [
        [r["event_id"] for r in batch],
        [r["ticket_id"] for r in batch],
        [r["customer_id"] for r in batch],
        [r["customer_name"] for r in batch],
        [r["event_type"] for r in batch],
        [r["latency_ms"] for r in batch],
        [r["event_time"] for r in batch],
        [r["_ingested_at"] for r in batch],
    ]
    con.execute(
        f"""
        insert into {TABLE}
